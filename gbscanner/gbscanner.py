...
class Start():
    ...
    def parse_and_exit(args):
        if len(args) == 3 and args[1] in ["--report", "--plot", "--csv", "--set"]:
            ...
            if args[1] == "--report":
                ...
                sys.exit("try using: gbscanner --report path/to/domain.com_yyyy_mm_dd_hh_mm_ss.json")
            ...
            if args[1] == "--plot":
                if args[2].endswith(".json"):
                    if path.isfile(args[2]):
                        ...
                        sys.exit()
                    sys.exit("report not found: %s" % args[2])
                sys.exit("try using: gbscanner --plot path/to/domain.com_yyyy_mm_dd_hh_mm_ss.json")
            ...
            if args[1] == "--csv":
                if args[2].endswith(".json"):
                    ...
                    sys.exit("report not found: %s" % args[2])
                sys.exit("try using: gbscanner --csv path/to/domain.com_yyyy_mm_dd_hh_mm_ss.json")
            ...
            if args[1] == "--set":
                ...
                #threads
                    ...
                    sys.exit("try using:\ngbscanner --set apikey-virustotal=APIKEY\ngbscanner --set timeout=SEC\ngbscanner --set threads=NUM")
    ...
    def arguments():
        ...
        description = "-"*80+"\n"
        description += "* SCAN\n"
        description += "full scan:\tgbscanner domain.com\n"
        description += "ignore code:\tgbscanner domain.com --no-http-code 404 500 530\n"
        description += "threads:\tgbscanner domain.com -th 50\n"
        description += "timeout:\tgbscanner domain.com -t 2\n\n"
        description += "* REPORT\n"
        description += "show report:\tgbscanner --report gbscanner_report/domain.com_yyyy_mm_dd_hh_mm_ss.json\n"
        description += "plot report:\tgbscanner --plot gbscanner_report/domain.com_yyyy_mm_dd_hh_mm_ss.json\n"
        description += "csv report:\tgbscanner --csv gbscanner_report/domain.com_yyyy_mm_dd_hh_mm_ss.json\n\n"
        description += "* SETTINGS\n"
        description += "set apikey:\tgbscanner --set apikey-virustotal=APIKEY\n"
        description += "set timeout:\tgbscanner --set timeout=sec\n"
        description += "set threads:\tgbscanner --set threads=num\n"
        description += "-"*80
        epilog = "warning:\tapikey virustotal is missing (https://www.virustotal.com/)\n\n" if not config["api"]["virustotal"] else "\n\n"
        epilog += "once you get gbscanner results, don't forget to use 'nmap' and 'dirsearch'\n\n"
        epilog += random.choice(Start.msg_rnd())

        parser = argparse.ArgumentParser(prog="gbscanner", description=description, epilog=epilog, formatter_class=RawTextHelpFormatter)
    ...
    def logo():
        return """
       _                                  
  __ _| |__ ___ __ __ _ _ _  _ _  ___ _ _ 
 / _` | '_ (_-</ _/ _` | ' \| ' \/ -_) '_|
 \__, |_.__/__/\__\__,_|_||_|_||_\___|_|  
 |___/                                    
""" 
def main():
